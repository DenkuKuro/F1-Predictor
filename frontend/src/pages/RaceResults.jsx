...
const RaceResults = ({ selectedRace }) => {
    ...
    // Reset when the selected race changes so the button reappears
    useEffect(() => {
        setSimData(null);
        setError('');
    }, [selectedRace]);
    ...
    const fetchSimulatedResults = async () => {
        ...
        if (!selectedRace || !selectedRace.race_id) {
            setError('Please select a race from the sidebar first.');
            return;
        }
        ...
        try {
            const response = await fetch(`/api/race_results?id=${userId}&race=${selectedRace.race_id}`);
            if (!response.ok) {
                const body = await response.json();
                throw new Error(body.error || 'Failed to fetch race results data');
            }
            const data = await response.json();
            setSimData({ ...data, metadata: selectedRace });
        } catch (err) {
            ...
            <article className='content-card'>
                ...
                {!simData && (
                    <button
                        ...
                        disabled={loading}
                    >
                        {loading ? 'Loading Results...' : 'View & Calculate Race Results'}
                    </button>
                )}
                    ...
                    <div style={{ marginTop: '2.5rem' }}>
                        ...
                        <div className="user-highlight" style={{ marginBottom: '2.5rem', padding: '1.5rem', background: 'rgba(225, 6, 0, 0.05)', border: '1px solid #e10600', borderRadius: '8px' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                                <h4 style={{ margin: 0, fontSize: '1.1rem' }}>⭐ Your Results: {simData.user_result.username}</h4>
                                <span style={{ fontSize: '1.25rem', fontWeight: 'bold', color: '#e10600' }}>{simData.user_result.score} pts</span>
                            </div>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: '10px', fontSize: '0.9rem' }}>
                                <p style={{margin:0}}><small style={{color:'#888'}}>P1:</small> {simData.user_result.prediction.p1}</p>
                                <p style={{margin:0}}><small style={{color:'#888'}}>P2:</small> {simData.user_result.prediction.p2}</p>
                                <p style={{margin:0}}><small style={{color:'#888'}}>P3:</small> {simData.user_result.prediction.p3}</p>
                                <p style={{margin:0}}><small style={{color:'#888'}}>SC:</small> {simData.user_result.prediction.safety_car ? 'Yes' : 'No'}</p>
                            </div>
                        </div>
                            ...
                            <table className="data-table">
                                ...
                                <tbody>
                                    {simData.leaderboard.map((user, index) => {
                                        const isCurrentUser = user.username === simData.user_result.username;
                                        return (
